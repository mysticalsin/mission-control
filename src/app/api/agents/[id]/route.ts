  ...
  try {
    ...
    if (removeWorkspace) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: `Failed to remove OpenClaw workspace for ${agent.name}: ${err instanceof Error ? err.message : 'unknown error'}` },
          { status: 502 }
        )
      }
    }
    ...
    try {
      ...
      configCleanupWarning = `OpenClaw config cleanup skipped for ${agent.name}: ${err instanceof Error ? err.message : 'unknown error'}`
