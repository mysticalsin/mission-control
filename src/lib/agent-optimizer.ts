...
export function analyzeToolPatterns(
  ...
  return {
    ...
    topTools: topTools.map((t: TopToolRow) => ({
...
export function generateRecommendations(
  ...
  if (agentCostTasks > 0 && fleetAvgCost > 0) {
    const agentCostPerTask = (agentCost?.cost ?? 0) / agentCostTasks
