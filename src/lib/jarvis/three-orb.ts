/**
 * ADR: Ported from tonys-jarvis orb.ts — Three.js WebGL particle field.
 * Context: Canvas 2D was used temporarily when THREE.PointsMaterial appeared
 *   saturated (white) during development; root cause was a transparent renderer
 *   clearing to white instead of the intended dark background.
 * Decision: Use setClearColor(0x050508, 1) (opaque dark) to match the original
 *   standalone design. AdditiveBlending works correctly on dark backgrounds.
 *   The panel stage already has bg-black, so the seam is invisible.
 * Visual: 2000 particles, 8000 connection lines, electrons during thinking,
 *   velocity physics, camera drift, transition tumble — identical to tonys-jarvis.
 * Trade-offs: Heavier than Canvas 2D; WebGL handles 2000 particles at 60fps fine.
 */
...
export function createThreeOrb(
  ...
  // ── Electrons — bright dots that travel along connections during thinking ──
  ...
  let lastState: ThreeOrbState = 'idle'
  ...
  function animate() {
    ...
    switch (state) {
      ...
      case 'disconnected':
        targetRadius = 30; targetSpeed = 0.1; targetBright = 0.25; targetSize = 0.3
        targetLineAmount = 0.05; targetElectronRate = 0; break
      case 'error':
        targetRadius = 20; targetSpeed = 0.6; targetBright = 0.7; targetSize = 0.4
    ...
    // Transition energy (tumble on state change)
  ...
  return {
    setState(s: ThreeOrbState): void { state = s },
    setAnalyser(a: AnalyserNode | null): void {
      analyser = a
