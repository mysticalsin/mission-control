...
export function initGnapRepo(repoPath: string): void {
  ...
  // Init git if not already a repo — stage only the files we created above
  const gitDir = path.join(repoPath, '.git')
  if (!fs.existsSync(gitDir)) {
    git(repoPath, ['init'])
    git(repoPath, ['add', '--', 'version', 'agents.json'])
...
function taskToGnapJson(task: McTask): GnapTask {
  ...
  if (Array.isArray(task.tags)) {
    ...
    try {
      const parsed: unknown = JSON.parse(task.tags || '[]')
      tags = Array.isArray(parsed) ? parsed : []
    } catch {
      logger.warn({ tags: task.tags, taskId: task.id }, 'Failed to parse task tags JSON, defaulting to empty array')
    }
  }
...
export function syncGnap(repoPath: string): SyncResult {
  ...
  if (hasRemote(repoPath)) {
    try {
      ...
      result.errors.push(`Pull failed: ${getErrorMessage(err)}`)
    }
  }
  ...
  if (hasRemote(repoPath) && hasChanges(repoPath)) {
    try {
      git(repoPath, ['add', '--', 'tasks'])
      git(repoPath, ['commit', '-m', `Sync from Mission Control at ${result.lastSync}`])
      git(repoPath, ['push'])
    } catch (err: unknown) {
      result.errors.push(`Push failed: ${getErrorMessage(err)}`)
    }
  }
