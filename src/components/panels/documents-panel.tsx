...
export function DocumentsPanel() {
  ...
  const loadTree = useCallback(async () => {
    ...
    setTreeError(null)
    try {
      const res = await fetch('/api/docs/tree', { signal: AbortSignal.timeout(8000) })
  ...
  const loadDoc = useCallback(async (path: string) => {
    ...
    try {
      const res = await fetch(`/api/docs/content?path=${encodeURIComponent(path)}`, { signal: AbortSignal.timeout(8000) })
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      setSearchError(null)
      try {
        const res = await fetch(`/api/docs/search?q=${encodeURIComponent(query)}&limit=100`, { signal: AbortSignal.timeout(8000) })
