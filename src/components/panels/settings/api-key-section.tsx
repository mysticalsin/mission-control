...
interface ApiKeySectionProps {
  ...
  rotating: boolean
  keyCopied: boolean
  onRotateConfirmOpen: () => void
  onRotateConfirmClose: () => void
  onRotateKey: () => void
  onCopyKey: () => void
  onDismissNewKey: () => void
}

/** API key management card rendered inside the Security settings tab. */
export function ApiKeySection({
  apiKeyInfo, apiKeyLoading, newApiKey, rotateConfirm, rotating, keyCopied,
  onRotateConfirmOpen, onRotateConfirmClose, onRotateKey, onCopyKey, onDismissNewKey,
}: ApiKeySectionProps) {
  return (
    <div className="space-y-3">
      <div className="bg-card border border-border rounded-lg p-4">
        ...
        {!rotateConfirm ? (
          <div className="mt-3">
            <Button onClick={onRotateConfirmOpen} variant="outline" size="sm">
              Rotate Key
            </Button>
          </div>
        ) : (
          <div className="mt-3 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={onRotateConfirmClose} variant="ghost" size="sm">
                Cancel
              </Button>
            </div>
          </div>
