...
// The essential panel IDs that remain visible in essential mode
const ESSENTIAL_PANEL_IDS = new Set(['overview', 'agents', 'tasks', 'chat', 'activity', 'logs', 'settings'])
  ...
  const handleChange = async (mode: 'essential' | 'full') => {
    ...
    try {
      ...
      if (mode === 'essential') {
        ...
        if (!ESSENTIAL_PANEL_IDS.has(store.activeTab)) {
