...
interface SettingsListProps {
  ...
  coordinatorPreviewFn: (configuredTarget: string) => string
  subscriptionDropdowns: Record<string, { label: string; value: string }[]>
  onEdit: (key: string, value: string) => void
  onReset: (key: string) => void
}

/** Convert snake_case key segment to Title Case label. */
...
/** Renders the list of editable settings rows for the active category. */
export function SettingsList({
  settings, edits, coordinatorTargetAgents, coordinatorPreviewFn,
  subscriptionDropdowns, onEdit, onReset,
}: SettingsListProps) {
  return (
    <div className="space-y-3">
      {settings.map(setting => {
        ...
        const coordinatorPreview = setting.key === 'chat.coordinator_target_agent'
          ? coordinatorPreviewFn(currentValue)
          : null
